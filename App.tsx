...
const dayToCity: Record<string, CityName> = {
...
const App: React.FC = () => {
  ...
  const downloadRecommendationsCSV = () => {
    ...
    Object.keys(locations).forEach((cityKey) => {
      ...
      const suggestions = locations[city].filter(l => l.type === 'suggestion' || l.day === 'Any');
      const topPickHotel = locations[city].find(l => l.type === 'hotel' && l.desc.includes("Top Pick"));
      
      suggestions.forEach(item => {
        const detailsCol = `[SUGGESTION] ${item.desc} | URL: ${item.url || 'N/A'}`;
